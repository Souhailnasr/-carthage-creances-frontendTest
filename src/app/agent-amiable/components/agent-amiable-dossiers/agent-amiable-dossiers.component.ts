...
@Component({
  selector: 'app-agent-amiable-dossiers',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
  ...
  templateUrl: './agent-amiable-dossiers.component.html',
  styleUrls: ['./agent-amiable-dossiers.component.scss']
})
export class AgentAmiableDossiersComponent implements OnInit, OnDestroy {
  ...
  displayedColumns: string[] = ['numeroDossier', 'titre', 'montantCreance', 'creancier', 'debiteur', 'statut', 'dateCreation', 'actions'];
  ...
  loadDossiers(agentId: number): void {
    this.loading = true;
    this.agentAmiableService.getDossiersAffectes(agentId, this.pageIndex, this.pageSize).pipe(
      takeUntil(this.destroy$)
    ).subscribe({
      next: (dossiers) => {
  ...
  manageActions(dossierId: number): void {
    // Navigation vers la gestion des actions
    window.location.href = `/agent-amiable/actions?dossierId=${dossierId}`;
  }
}
