...
@Component({
  selector: 'app-dossiers-juridique',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './dossiers-juridique.component.html',
  styleUrls: ['./dossiers-juridique.component.scss']
})
export class DossiersJuridiqueComponent implements OnInit, OnDestroy {
  dossiers: DossierApi[] = [];
  ...
  displayedColumns: string[] = ['numeroDossier', 'titre', 'montantCreance', 'creancier', 'debiteur', 'predictionIA', 'avocat', 'huissier', 'urgence', 'statut', 'dateCreation', 'actions'];
  ...
  loadDossiers(): void {
    ...
    this.dossierApiService.getDossiersRecouvrementJuridique(this.pageIndex, this.pageSize, sort).pipe(
      takeUntil(this.destroy$),
  ...
  calculateStatistics(): void {
    ...
    this.dossiersAvecAvocat = this.dossiers.filter(d => d.avocat).length;
    this.dossiersAvecHuissier = this.dossiers.filter(d => d.huissier).length;
  ...
  applyFilters(): void {
    ...
    if (this.searchTerm.trim()) {
      ...
      filtered = filtered.filter(d => 
        ...
        d.avocat?.nom?.toLowerCase().includes(term) ||
        d.huissier?.nom?.toLowerCase().includes(term)
      );
    }
  ...
  exportToCSV(): void {
    const headers = ['Numéro', 'Titre', 'Montant', 'Créancier', 'Débiteur', 'Avocat', 'Huissier', 'Urgence', 'Statut', 'Date Création'];
    ...
    link.setAttribute('download', `dossiers-juridique-${new Date().toISOString().split('T')[0]}.csv`);
